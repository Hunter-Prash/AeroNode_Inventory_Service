...
function issueAccessToken(user: { id: string }) {
    const payload: JwtAccessPayload = { sub: user.id };
...
function issueRefreshToken(user: { id: string }) {
...
function assertEmailPassword(body: any): { email: string; password: string } | null {
...
function requireAuth(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        const payload = jwt.verify(token, ACCESS_TOKEN_SECRET) as JwtAccessPayload;
...
async function findValidRefreshTokenRows(userId: string) {
...
authRouter.post("/refresh", async (req: Request, res: Response) => {
    ...
    try {
        payload = jwt.verify(token, REFRESH_TOKEN_SECRET) as JwtRefreshPayload;
    } catch {
 ...
 */
authRouter.post("/me/change-password", requireAuth, async (req: Request, res: Response) => {
    const userId = (req as any).user.sub as string;
